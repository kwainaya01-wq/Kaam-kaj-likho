...
document.getElementById('mainAuthBtn').onclick = () => {
    ...
    if(!email || !password) return myAlert("Email/Password dalo!");
...
document.getElementById('saveBtn').onclick = async () => {
    ...
    if (!name || !dateValue) return myAlert("Naam aur Date zaroori hai!");
    ...
    try {
        ...
        myAlert("Kaam Save Ho Gaya!");
    } catch (e) { myAlert(e.message); } // FIX 1: Typo 'myAalert' fixed to 'myAlert'
};
...
window.downloadPDF = (workerId, workerName) => {
    try {
        ...
        myAlert("PDF download shuru ho gaya!");
    } catch (err) {
        console.error(err);
        // FIX 2: Browser alert changed to myAlert for consistency
        myAlert("PDF Error: Ek baar internet check karein."); 
    }
};
...
// --- CUSTOM ALERT LOGIC (GLOBAL) ---
window.myAlert = function(msg, isConfirm = false, callback = null) {
